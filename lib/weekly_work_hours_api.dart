...
class WeeklyWorkHoursApi {
  // Singleton instance
  static final WeeklyWorkHoursApi _instance = WeeklyWorkHoursApi._internal();
  factory WeeklyWorkHoursApi() => _instance;
  WeeklyWorkHoursApi._internal();
  ...
  void startPeriodicUpdates(String empKey,
    ...
    // Fetch immediately
    fetchWeeklyWorkHours(empKey).then((result) {
  ...
  // Fetch weekly work hours data
  Future<Map<String, dynamic>> fetchWeeklyWorkHours(String empKey) async {
    try {
      ...
      try {
            ...
            if (data.containsKey('status')) {
              ...
              if (success && data.containsKey('work_hours')) {
                ...
                return {
                  'success': false,
                  'message':
                      data['message'] ?? 'No weekly work hours data available',
                  'raw_response': data,
                };
              }
            }
        ...
        try {
          ...
          return {
            'success': false,
            'message': data['message'] ?? 'Failed to get weekly work hours',
            'raw_response': data,
          };
        } catch (e) {
          return {
