...
class LeaveApplicationsApi {
  ...
  Future<Map<String, dynamic>> validateSubmit({
    ...
    try {
      ...
      try {
        final uri =
            '$apiUrl?emp_key=${Uri.encodeComponent(bodyMap['emp_key']!)}&leave_type=${Uri.encodeComponent(bodyMap['leave_type']!)}&creator_owner=${Uri.encodeComponent(bodyMap['creator_owner']!)}';
        final resp = await http.get(Uri.parse(uri), headers: {
          'Accept': 'application/json'
        }).timeout(const Duration(seconds: 15));
        if (resp.statusCode == 200) {
